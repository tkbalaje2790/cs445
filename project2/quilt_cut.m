...
for y = 1:N
    for x = 1:N
        ...
        elseif(x == 1)
            ...
            %figure(1)
            %subplot(1,3,1)
            %imagesc(overlap_left), axis image
            %subplot(1,3,2)
            %imagesc(overlap_right), axis image
            %subplot(1,3,3)
            %magesc(vertical_error), axis image, colormap gray
            %hold on
            %[cut_y,cut_x] = draw_line(vertical_error,cut_mask_vert)
            %plot(cut_y, cut_x,'r','LineWidth',2);
            %hold off
            %--
